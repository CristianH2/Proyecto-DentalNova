...
namespace Proyecto_DentalNova.Controllers
{
    public class OdontologoController : Controller
    {
        ...
        {
            filtro.EspecialidadesDisponibles = await _context.Especialidades
                .OrderBy(e => e.Nombre)
        ...
        public async Task<IActionResult> Index([Bind(Prefix = "Filtro")] OdontologoFilterViewModel filtro)
        {
            ...
            IQueryable<Odontologo> query = _context.Odontologos
                .Include(o => o.Usuario)
                .Include(o => o.Especialidades)
                .AsNoTracking();
            ...
            // Solo mostrar odontólogos con usuario activo
            query = query.Where(o => o.Usuario.Activo);

            query = query.OrderBy(o => o.Usuario.Apellidos);

            var pagedResults = await PaginatedList<Odontologo>.CreateAsync(query, filtro.Page, filtro.PageSize);
